...
description: |
  Proven workflow architectural patterns from real n8n workflows, bao gồm các patterns tích hợp CDS Platform.
  Use when: building new workflows, designing workflow structure, choosing workflow patterns,
  planning workflow architecture, tích hợp CDS Platform với hệ thống bên ngoài,
  or asking about webhook processing, HTTP API integration, database operations, AI agent workflows, or scheduled tasks.
  Trigger: "n8n", "workflow", "automation", "webhook", "scheduled task",
  "tích hợp hệ thống", "integration pattern", "API integration", "ETL".
---
...
### 5 Workflow Phổ Biến Khi Triển Khai CDS

| # | Workflow | Pattern | Mô tả |
|---|---------|---------|-------|
| 1 | **CDS ↔ ERP Sync** | Database Operations | Đồng bộ dữ liệu khách hàng, đơn hàng, tồn kho giữa CDS và SAP/Oracle/Exact |
| 2 | **Lead Capture → CDS CRM** | Webhook Processing | Nhận lead từ website/landing page → tạo contact trong CDS Sales Management |
| 3 | **Daily KPI Report** | Scheduled Tasks | Hàng ngày lấy metrics từ CDS BI → tổng hợp → gửi email/Zalo cho Manager |
| 4 | **AI Customer Support** | AI Agent Workflow | Chatbot tích hợp CDS CDP data → trả lời câu hỏi khách hàng tự động |
| 5 | **Order → Inventory → Invoice** | HTTP API Integration | Khi có đơn hàng mới → cập nhật tồn kho → tạo hóa đơn tự động |

### CDS Integration Architecture

```
External Systems          n8n Workflows              CDS Platform
─────────────          ──────────────              ────────────
Website/App    ──→  Webhook Processing  ──→  M1: Sales Management
SAP/Oracle     ←→  Database Operations ←→  M5: Inventory
Email/Zalo     ←──  Scheduled Tasks    ←──  M10: BI & Reports
Chatbot        ──→  AI Agent Workflow   ──→  M4: CDP
Payment GW     ──→  HTTP API Integration──→  M9: Accounting
```

### Lưu Ý Khi Tích Hợp CDS

1. **Authentication**: CDS Platform dùng API Key + OAuth2 - cấu hình trong n8n Credentials
2. **Rate Limiting**: Tối đa 100 requests/phút cho CDS API - dùng Split in Batches
3. **Data Mapping**: Luôn dùng Set node để map field names giữa CDS và hệ thống khác
4. **Error Handling**: Tất cả CDS workflows PHẢI có Error Trigger → thông báo qua Zalo/Email
5. **Monitoring**: Ghi log mỗi sync vào CDS hoặc Google Sheets để audit
