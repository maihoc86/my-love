...
### 4.2.1. SCR-02.1: All Notes Screen — Tất Cả Ghi Chú

**Mô tả**: Màn hình danh sách đầy đủ tất cả ghi chú. Truy cập từ "Xem tất cả >" trên Dashboard. Hỗ trợ tìm kiếm real-time và lọc theo danh mục.

**Route**: `/entries/all` (hoặc `app/entries/all.tsx`)

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-AL01 | Header: back arrow, title "Tất cả ghi chú", sort icon | P0 |
| FR-AL02 | Search bar sticky dưới header: placeholder "Tìm kiếm ghi chú...", icon 🔍, nút clear (×), real-time filter theo title + detail | P0 |
| FR-AL03 | Filter chips horizontal scroll: "Tất cả X" (active/default) + chip cho từng category có entries (icon + count) | P0 |
| FR-AL04 | Active chip: bg-primary text-white; Inactive: bg-white border text-slate-600 | P0 |
| FR-AL05 | Results count: "X ghi chú" (left) + "Mới nhất ▾" sort trigger (right, text-primary) | P0 |
| FR-AL06 | Entry card: icon circle 44px + title + detail (optional, truncate 1 line) + category·timeAgo + "Chi tiết" button | P0 |
| FR-AL07 | Entry card: bg-white, rounded-2xl, shadow-sm, mx-4 mb-3 | P0 |
| FR-AL08 | FAB bottom-right: 56×56px, bg-primary, icon (+), shadow rose → navigate to Add Entry | P0 |
| FR-AL09 | Sort bottom sheet: handle bar + "Sắp xếp & Lọc" title + radio group (Mới nhất/Cũ nhất/A→Z) + "Áp dụng" button | P1 |
| FR-AL10 | Empty state: icon 🔍 64px + "Không tìm thấy ghi chú" + "Thử tìm với từ khoá khác" + nút "Xoá bộ lọc" | P0 |
| FR-AL11 | Kết quả tìm kiếm highlight từ khoá khớp trong title (bold/rose color) | P2 |
| FR-AL12 | Pull-to-refresh để reload từ Supabase | P1 |
| FR-AL13 | Skeleton loading (3 placeholder cards) khi đang fetch | P1 |

**Interaction Flow**:
```
Dashboard "Xem tất cả >"
  → All Notes (danh sách đầy đủ, filter = "Tất cả")
    → Gõ vào search bar → debounce 300ms → filter local data
    → Tap filter chip → filter theo category
    → Tap sort icon → bottom sheet → chọn sort → "Áp dụng"
    → Tap "Chi tiết" → Entry Detail screen (future)
    → Tap FAB (+) → Add Entry screen
```

**API**:
```
GET /entries?user_id=eq.{uid}&order=created_at.desc          ← initial load
GET /entries?user_id=eq.{uid}&category=eq.{cat}              ← category filter
GET /entries?user_id=eq.{uid}&or=(title.ilike.%{q}%,detail.ilike.%{q}%)  ← search
```
...
type FilterState = {
  ...
  activeCategory: string;    // "all" | category enum
...
### 4.5b. SCR-05.1: Partner Info Screen (Chỉnh sửa thông tin người yêu)

**Mô tả**: Form chỉnh sửa thông tin người yêu Thái Học.

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-PI01 | Header sticky: back (←), title "Thông tin người yêu", nút "Lưu" rose bên phải | P0 |
| FR-PI02 | Avatar 96px tròn, camera icon overlay góc dưới phải để đổi ảnh | P0 |
| FR-PI03 | Text "Chạm để thay đổi ảnh" bên dưới avatar | P1 |
| FR-PI04 | Form card trắng: tên gọi (User icon), biệt danh (Heart icon), sinh nhật (Cake icon, date picker), số điện thoại (Phone icon), ghi chú (textarea 3 dòng) | P0 |
| FR-PI05 | Sinh nhật hiển thị readonly với calendar icon bên phải | P0 |
| FR-PI06 | Nút "Lưu thay đổi" rose gradient, full width, shadow | P0 |
| FR-PI07 | Nút "Xoá tài khoản" text đỏ nhỏ, Alert confirm 2 bước | P1 |
| FR-PI08 | TODO: PUT /auth/v1/user để lưu lover_name, lover_birthday, lover_nickname | P0 |

### 4.5c. SCR-05.2: Personal Info Screen (Thông tin cá nhân)

**Mô tả**: Xem và chỉnh sửa thông tin tài khoản cá nhân.

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-PN01 | Header sticky: back (←), title "Thông tin cá nhân", nút "Chỉnh sửa" / "Hủy" | P0 |
| FR-PN02 | Profile hero card gradient rose→purple: avatar 80px, tên, email | P0 |
| FR-PN03 | Form card: Họ và tên, Email, Số điện thoại (readonly mặc định), Ngày tham gia (luôn readonly) | P0 |
| FR-PN04 | Khi bấm "Chỉnh sửa": fields chuyển editable, border rose, focus state | P0 |
| FR-PN05 | Khi bấm "Hủy": revert về giá trị ban đầu, readonly | P0 |
| FR-PN06 | Nút "Lưu thay đổi" slide up từ bottom khi edit mode, ẩn khi readonly | P0 |
| FR-PN07 | TODO: PATCH Supabase auth.updateUser({ full_name, phone }) | P0 |

### 4.5d. SCR-05.3: Security Screen (Bảo mật)

**Mô tả**: Quản lý bảo mật tài khoản.

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-SC01 | Header sticky: back (←), title "Bảo mật" | P0 |
| FR-SC02 | Section "Xác thực": toggle sinh trắc học (Fingerprint icon, default ON), toggle OTP 2 bước (Shield icon, default OFF) | P0 |
| FR-SC03 | Nav row "Đổi mật khẩu" → navigate to /(auth)/forgot-password | P0 |
| FR-SC04 | Section "Phiên đăng nhập": device card với tên thiết bị, badge xanh "Thiết bị này", status "Đang hoạt động" | P0 |
| FR-SC05 | Nút "Đăng xuất tất cả thiết bị khác" — orange, border, Alert confirm | P1 |
| FR-SC06 | Section "Nguy hiểm": nav row đỏ "Xoá tài khoản" với subtitle, Alert confirm 2 bước | P0 |
| FR-SC07 | TODO: Supabase signOut(scope:'others') cho đăng xuất thiết bị khác | P1 |
| FR-SC08 | TODO: Supabase auth.admin.deleteUser() cho xoá tài khoản | P1 |

### 4.5e. SCR-05.4: Backup Screen (Sao lưu & Đồng bộ)

**Mô tả**: Quản lý sao lưu và đồng bộ dữ liệu.

| ID | Requirement | Priority |
|----|-------------|----------|
| FR-BK01 | Header sticky: back (←), title "Sao lưu & Đồng bộ" | P0 |
| FR-BK02 | Status hero card rose gradient: cloud icon, "Đã đồng bộ" bold, timestamp lần cuối, nút "Đồng bộ ngay" trắng | P0 |
| FR-BK03 | Section "Cài đặt sao lưu": toggle tự động (Refresh icon, ON), toggle Wi-Fi only (Wifi icon, ON) | P0 |
| FR-BK04 | Nav row "Tần suất" → value "Hàng ngày" | P1 |
| FR-BK05 | Info row "Dung lượng": "2.4 MB / 500 MB" + progress bar rose bên dưới | P0 |
| FR-BK06 | Section "Lịch sử sao lưu": 3 items với clock icon, timestamp, dung lượng, badge "Tự động"/"Thủ công" | P0 |
| FR-BK07 | Nút "Xoá tất cả bản sao lưu" — red border, Alert confirm | P1 |
| FR-BK08 | TODO: Supabase Storage cleanup cho xoá cache/backup | P1 |
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-DM01 | eKMap full-width map area (52% screen height) với map pins theo category | P0 |
| FR-DM02 | Category filter pills floating trên map: Quán ăn, Cà phê, Homestay, Khu vui chơi, TTTM. Active = bg-primary | P0 |
| FR-DM03 | Map pins: primary pin lớn (location_on 36px) với mini icon bên trong, other pins 24px opacity 60% | P0 |
| FR-DM04 | User location: blue dot 16px với white border + pulse-blue animation | P0 |
| FR-DM05 | Zoom controls: my_location, add, remove — trục đứng, bg-white rounded-xl, góc phải map | P0 |
| FR-DM06 | "Powered by eKMap" watermark góc trái dưới map | P0 |
| FR-DM07 | AI gợi ý badge trong bottom sheet: auto_awesome icon + text italic gray | P1 |
| FR-DM08 | Bottom sheet (-mt-4, rounded-t-3xl, shadow-lg): drag handle + spot name + rating + category + distance + address | P0 |
| FR-DM09 | Tags: pills outline (rose "Em thích", blue "Có chỗ đỗ xe", gray "Bình dân") với icon | P1 |
| FR-DM10 | Action buttons: "Dẫn đường" col-span-2 primary, Lưu icon, Share icon (4-col grid) | P0 |
| FR-DM11 | "Gợi ý gần đây" section: list item với colored icon box 48px rounded-2xl + tên + category+distance + chevron | P1 |
| FR-DM12 | Tap spot in nearby list → update selected spot in bottom sheet | P0 |
| FR-DM13 | Map area simulated với radial-gradient dot pattern (map-grid style) + water blob + road lines | P1 |

**Layout cụ thể (từ Stitch)**:
```
SafeAreaView flex-1
├── Header (backdrop-blur, border-b): back btn + title "Bản đồ hẹn hò" + filter btn
├── Map Area (flex: 1 height ~52%)
│   ├── Simulated map background (grid + roads + water)
│   ├── Category pills (absolute top-4, horizontal scroll)
│   ├── User location dot (absolute center)
│   ├── Primary pin (absolute, larger, with mini-icon)
│   ├── Other pins (absolute, smaller, 60% opacity)
│   ├── Zoom controls (absolute right-4 bottom-12)
│   └── Watermark (absolute bottom-6 left-2)
└── Bottom Sheet (-mt-4, rounded-t-3xl, flex-1, overflow-y-auto)
    ├── Drag handle
    ├── AI suggestion badge (gray bg, auto_awesome icon)
    ├── Spot name + rating + category + distance
    ├── Address
    ├── Tags pills
    ├── Action buttons (4-col grid)
    ├── Divider
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-AL01 | Stats bar: single white card rounded-3xl, 3 columns với divider: "Tổng ảnh" / "Sự kiện" / "Tháng" | P0 |
| FR-AL02 | Filter tabs horizontal scroll: Tất cả (primary), Hẹn hò, Du lịch, Kỷ niệm, Yêu thích | P0 |
| FR-AL03 | Photo groups: title (bold) + date format "DD/MM/YYYY • N ảnh" + "Xem tất cả" link | P0 |
| FR-AL04 | Mixed grid (2fr 1fr): large photo row-span-2 trái, 2 small photos phải. "+N" overlay trên ảnh cuối | P0 |
| FR-AL05 | Triple grid (1fr 1fr 1fr) cho nhóm du lịch, height 96px | P0 |
| FR-AL06 | Floating camera FAB: bottom-8 right-6, w-16 h-16, rose gradient, add_a_photo icon | P0 |
| FR-AL07 | Upload from camera or gallery (expo-image-picker) | P0 |
| FR-AL08 | "Xem tất cả" per group → full gallery view | P1 |
| FR-AL09 | Photo cells dùng colored background + icon khi chưa có ảnh thật | P0 |
| FR-AL10 | Header: back btn + title "Album ảnh" + camera btn (primary) | P0 |

**Photo Group layout (từ Stitch)**:
```
Group Header: [title + date•count] [Xem tất cả →]
Mixed Grid (date/anniversary groups):
  ┌─────────────────┬──────────┐
  │  large photo    │ sm photo │  row-span-2
  │  (2fr)          ├──────────┤
  │                 │ sm +N    │
  └─────────────────┴──────────┘
Triple Grid (travel groups):
  ┌──────┬──────┬──────┐
  │  img │  img │ +N   │  height 96px
  └──────┴──────┴──────┘
```
...
| ID | Requirement | Priority |
|----|-------------|----------|
| FR-IN01 | Summary pill badge: bg-purple/10, border-purple/20, analytics icon, "56 ghi chú · 11 danh mục · 5 cảm xúc" | P0 |
| FR-IN02 | Neuron map: rounded-3xl card (~55vh), white/50 bg, shadow-inner. SVG connection lines rose→purple gradient | P0 |
| FR-IN03 | Center node: 96px circle rose gradient với animate-pulse-slow. Text "THÁI HỌC" + "Trung tâm" | P0 |
| FR-IN04 | Category nodes: circular, size proportional to count, positioned organically. float animation với animation-delay stagger | P0 |
| FR-IN05 | Sub-labels dưới node: bg-orange-50 pill với text content preview (VD: "Phở bò, Mắm tôm") cho Món ăn | P1 |
| FR-IN06 | Warning badge (-top-1 -right-1) cho node Dị ứng | P0 |
| FR-IN07 | Sentiment analysis bars: bg-white card, 4 rows với label + % + progress bar (2px height) | P0 |
| FR-IN08 | AI Tổng quan card: gradient border (purple→pink), auto_awesome icon, paragraph analysis + gợi ý hẹn hò | P1 |
| FR-IN09 | Header: back btn (white circle shadow) + title "Insight 360°" + filter btn | P0 |
| FR-IN10 | Floating animation on category nodes dùng Animated.loop + Animated.sequence | P1 |

**Node positions (từ Stitch)**:
```
SVG ViewBox: 320 x 300
Center: (160, 150) - Thái Học
Nodes (6 categories):
  - Món ăn (24):  top-left    ~(55, 60)
  - Địa điểm (8): top-right   ~(270, 75)
  - Sở thích (15):right-mid   ~(275, 165)
  - Âm nhạc (6):  bottom-ctr  ~(160, 265)
  - Dị ứng (4):   bottom-left ~(45, 240)
  - Tính cách (7):left-mid    ~(30, 120)
Size formula: r = 16 + count * 0.6 (min 16, max 30)
```
